...
def quotes_historical_google(symbol, start_date, end_date):
    ...
    url = 'http://finance.google.com/finance/historical?' + urlencode(params)
    print("url: ", url)
    ...
    # with open('%s.csv'%filename, mode='w') as fd:
    #     print(response.text)
    #     fd.write(response.text)
    data = np.genfromtxt('%s.csv'%filename, delimiter=',', skip_header=1,
